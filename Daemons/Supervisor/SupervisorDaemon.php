...
abstract class SupervisorDaemon
{
    ...
    static protected $_sigHandlers = [
        SIGHUP => array(__NAMESPACE__, 'defaultSigHandler'),
        SIGINT => array(__NAMESPACE__, 'defaultSigHandler'),
        SIGUSR1 => array(__NAMESPACE__, 'defaultSigHandler'),
        SIGUSR2 => array(__NAMESPACE__, 'defaultSigHandler'),
        SIGTERM => array(__NAMESPACE__, 'defaultSigHandler')
    ];
    ...
    public static function getDaemons()
    {
        if (empty(static::$daemons)) {
            static::$daemons = [];
            $path = realpath(__DIR__);
            foreach (new \DirectoryIterator($path) as $fileInfo) {
                if (!$fileInfo->isDot() && !$fileInfo->isDir()) {
                    $class = '\\' . __namespace__ .'\\' . str_replace('.php', '', $fileInfo->getFilename());
                    $name = $class::getName();
                    if (!empty($name)) {
                        static::$daemons[$name] = $class;
                    }
                }
            }
        }
        return static::$daemons;
    }
    ...
    {
        $shell = new \Imin\Helpers\ShellHelper();
    ...
    {
        \AppBundle\Services\Debug::error($error);
    }
