...
class InstallSupervisorDaemonCommand extends ContainerAwareCommand {
    ...
    protected function interact(InputInterface $input, OutputInterface $output) {
        ...
        if (!$input->getOption('all') && empty($daemonName)) {
            $daemons = SupervisorDaemon::getDaemons();
            $names = [];
            $output->writeln('');
            ...
            if ($uninstall) {
                $question = new Question('Please enter the name of the daemon to uninstall: ');
            } else {
                $question = new Question('Please enter the name of the daemon to (re-) install: ');
            }
